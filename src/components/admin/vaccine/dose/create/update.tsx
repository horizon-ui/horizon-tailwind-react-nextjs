...
const UpdateDose = ({ handleEditDose }) => {
  ...
  const [dose, setDoseRecoil] = useRecoilState(doseState);

  const initialFormData = {
    name: dose?.name,
    doseDuration: dose?.doseDurationData,
  ...
  const updateDose = useUpdateDose({
    onSuccess: (resp: AxiosResponse) => {
      if (resp && resp.status === 200) {
        warningAlert2('Dose updated successfully');
